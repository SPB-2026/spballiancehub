...
export default function Leaderboard() {
  const { data, loading, error, reload } = useAsync(() => api.get('/leaderboard?metric=score'), []);
            ...
            {[podium[1], podium[0], podium[2]].filter(Boolean).map((r) => {
              ...
              return (
                <div key={r.id} className={`card pod${isFirst ? ' pod-1' : ''}${r.rank === 2 ? ' pod-2' : ''}${r.rank === 3 ? ' pod-3' : ''}`}>
                  ...
                  <TownCenterBadge level={r.town_center} size={40} className="pod-tc" />
                </div>
              );
            })}
          </div>
              ...
              <tbody>
                {rest.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td><TownCenterBadge level={r.town_center} size={32} /></td>
