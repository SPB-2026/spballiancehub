...
export function setSessionToken(token) {
  try { token ? localStorage.setItem(SESSION_KEY, token) : localStorage.removeItem(SESSION_KEY); } catch { /* ignore */ }
}
...
async function request(method, path, body) {
  const send = async (withCsrf) => {
    ...
    if (method !== 'GET' && method !== 'HEAD') {
      if (withCsrf) {
        // Cookie channel: double-submit CSRF token. (Bearer channel skips CSRF server-side.)
  ...
  try {
    ...
    if (err instanceof ApiError && err.status === 403 && /csrf|token/i.test(err.message)) {
      csrfToken = null;
      try {
...
export default {
  ...
  async upload(path, file, field = 'file') {
    ...
    if (!getSessionToken()) headers['X-CSRF-Token'] = await getCsrf();
