...
async function imageUrl(value) {
  ...
  if (!(await Media.findByUrl(s))) throw httpError(400, 'Event image must come from the Media library.');
  return s;
}
...
async function create(input) {
  ...
  const image = await imageUrl(input.image);
...
async function update(id, input) {
  ...
  if (input.image !== undefined) fields.image = await imageUrl(input.image);
