...
function csrfToken(req, res) {
  ...
  // In production the frontend is cross-origin (Vercel -> Render), so
  // SameSite=None is required for the cookie to be sent on fetch.
  // Lax would be stripped on cross-site subrequests. In dev the proxy
  // makes everything same-origin so Lax is fine.
  const sameSite = env.IS_PROD ? 'none' : 'lax';
  res.cookie('spb_csrf', fresh, { httpOnly: true, sameSite, maxAge: 12 * 3600 * 1000, secure: env.IS_PROD });
  return fresh;
}
...
function csrfProtect(req, res, next) {
  ...
  if (header) {
    try {
      ...
      const sameSite = env.IS_PROD ? 'none' : 'lax';
      res.cookie('spb_csrf', header, { httpOnly: true, sameSite, maxAge: 12 * 3600 * 1000, secure: env.IS_PROD });
      return next();
